...
* **Key Features:**
  ...
  - Fronted by `calcrony.app` / `api.calcrony.app` (test: `test.` / `api-test.`); the old `*.theguywiththedogs.dev` names 301 to the new ones so links the bot already posted keep working. OAuth callbacks (`/auth/discord/callback`, `/oauth/google/callback`) live on the API host and must be registered for it.
